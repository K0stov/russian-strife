        ...
        if(!player->inventorydown)
        {
            ...
            else if(cmd->buttons2 & BT2_INVDROP)
            {
                ...
                if(workparm)
                {
                    ...
                    if(cheat & CF_NOCLIP)
                    {
                        player->message = DEH_String("ghj[j;ltybt xthtp cntys drk>"); // [rus] Прохождение через стены вкл.
                        player->mo->flags |= MF_NOCLIP;
                    }
                    else
                    {
                        player->mo->flags &= ~MF_NOCLIP;
                        player->message = DEH_String("ghj[j;ltybt xthtp cntys jnrk>"); // [rus] Прохождение через стены откл.
                    }
                }
            }
        }
...
boolean P_UseInventoryItem(player_t* player, int item)
{
    ...
    for(i = 0; i < player->numinventory; i++)
    {
        ...
        M_snprintf(useinventorymsg, sizeof(useinventorymsg),
                   "bcgjkmpjdfy ghtlvtn: %s>", name); // [rus] Использован предмет: %s.
