...
void M_DrawNameChar(void)
{
    ...
    M_WriteText(72, 28, DEH_String("ddtlbnt bvz gthcjyf;f")); // [rus] Введите имя персонажа
...
void M_SaveGame (int choice)
{
    ...
    if (netgame)
    {
        ...
        M_StartMessage(DEH_String("d ctntdjq buht ytkmpz cj[hfyznmcz!"), NULL, false); // [rus] В сетевой игре нельзя сохраняться!
        return;
    }
    if (!usergame)
    {
...
void M_AutoUseHealth(void)
{
    if(!netgame && usergame)
    {
        ...
        if(players[consoleplayer].cheats & CF_AUTOHEALTH)
            players[consoleplayer].message = DEH_String("fdnjvfnbxtcrjt ktxtybt drk>"); // [rus] Автоматическое лечение ВКЛ.
        else
            players[consoleplayer].message = DEH_String("fdnjvfnbxtcrjt ktxtybt jnrk>"); // [rus] Автоматическое лечение ОТКЛ.
    }
}
...
void M_ChangeShowText(void)
{
    ...
    if(dialogshowtext)
        players[consoleplayer].message = DEH_String("ntrcn d lbfkjuf[ drk>"); // [rus] Текст в диалогах ВКЛ.
    else
        players[consoleplayer].message = DEH_String("ntrcn d lbfkjuf[ jnrk>"); // [rus] Текст в диалогах ОТКЛ.
}
...
void M_QuitStrife(int choice)
{
    DEH_snprintf(endstring, sizeof(endstring),
                 "ds ltqcndbntkmyj [jnbnt\ndsqnb bp buhs?\n\n" DOSY); // [rus] Вы действительно хотите\nвыйти из игры?
