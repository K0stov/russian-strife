    ...
    if(!pickupmsg[0])
    {
        if(special->info->name)
        {
            DEH_snprintf(pickupmsg, sizeof(pickupmsg), 
                         "gjkexty ghtlvtn: %s>", DEH_String(special->info->name)); // [rus] Получен предмет: %s.
        }
        else
            DEH_snprintf(pickupmsg, sizeof(pickupmsg), "gjkexty ghtlvtn>");  // [rus] Получен предмет.
    }
    ...
    */
    ...
    if(source && source->player)
    {
        ...
        if(target->player)
        {
            ...
            DEH_snprintf(plrkilledmsg, sizeof(plrkilledmsg),
                         "buhjr %s e,bk buhjrf %s", // [rus] Игрок %s убил игрока %s
    ...
    case MT_TOKEN_SHOPCLOSE:
        ...
        M_snprintf(plrkilledmsg, sizeof(plrkilledmsg),
                   "%s", DEH_String("ns gjlyzk nhtdjue! ujnjdmcz r cvthnb!")); // [rus] Ты поднял тревогу! Готовься к смерти!
        if(!deathmatch)
            players[consoleplayer].message = plrkilledmsg;
    ...
    case MT_TOKEN_ALARM:
        ...
        M_snprintf(plrkilledmsg, sizeof(plrkilledmsg),
                   "%s", DEH_String("ukegtw> ns drk.xbk cbuyfkbpfwb.!")); // [rus] Глупец. Ты включил сигнализацию!
        if(!deathmatch)
            players[consoleplayer].message = plrkilledmsg;
        return;
