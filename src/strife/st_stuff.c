...
boolean ST_Responder(event_t* ev)
{
    ...
    else if (cht_CheckCheat(&cheat_dev, ev->data2))
    {
        devparm = !devparm;
        if (devparm)
            plyr->message = DEH_String("ht;bv hfphf,jnxbrf drk>"); // [rus] Режим разработчика вкл.
        else
            plyr->message = DEH_String("ht;bv hfphf,jnxbrf jnrk>"); // [rus] Режим разработчика откл.
    }
    ...
    else if(cht_CheckCheat(&cheat_stealth, ev->data2))
    {
        ...
        if(plyr->cheats & CF_NOMOMENTUM)
            plyr->message = DEH_String("cntkc-cfgjub yfltns"); // [rus] Стелс-сапоги надеты
        else
            plyr->message = DEH_String("cntkc-cfgjub cyzns"); // [rus] Стелс-сапоги сняты
    }
    ...
    if(cht_CheckCheat(&cheat_powerup[ST_PUMPUP_H], ev->data2))
    {
        ...
        plyr->message = DEH_String("gjkextys ghtlvtns!"); // [rus] Получены предметы!
    }
    ...
    else if(cht_CheckCheat(&cheat_scoot, ev->data2))
    {
        ...
        if(spot <= 10) 
        {
            plyr->message = DEH_String("gthtvtotybt>>>"); // [rus] Перемещение...
            G_RiftCheat(spot);
            return false;
        }
    }
    ...
    {
        stonecold ^= 1;
        plyr->message = DEH_String("e,tq b[> e,tq b[ dct[>"); // [rus] Убей их. Убей их всех.
        return false;
    }
    ...
    {
        plyr->message = DEH_String("ns ghtdhfoftim dct d pjkjnj< ltnrf!"); // [rus] ТЫ ПРЕВРАЩАЕШЬ ВСЕ В ЗОЛОТО, ДЕТКА!
