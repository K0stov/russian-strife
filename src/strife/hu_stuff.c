...
#define HU_INPUTY       (HU_MSGY + HU_MSGHEIGHT*(SHORT(hu_font[0]->height) +2)) // [rus] Изменено вертикальное расположение для отображения буквы Й
        ...
        else
        {
            ...
            else
            {
                ...
                if(i < MAXPLAYERS)
                {
                    ...
                    else
                    {
                        ...
                        DEH_snprintf(lastmessage, sizeof(lastmessage),
                            "cjj,otybt buhjre: %c", '1' + i); // [rus] Сообщение игроку:
                        plr->message = lastmessage;
                    }
                }
                else if(c == '$') // [STRIFE]: name changing
                {
                    ...
                    M_StringCopy(lastmessage, DEH_String("cvtyf bvtyb:"), // [rus] Смена имени:
                                 sizeof(lastmessage));
                    plr->message = lastmessage;
