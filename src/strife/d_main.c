...
void D_SetGameDescription(void)
{
    ...
    W_MergeFile("rus_files/strife-russian.wad");
    W_MergeFile("rus_files/strife-russian-voices.wad");
}
...
{
    "===========================================================================\n"
    "Внимание: данная версия STRIFE подверглась изменениям.\n"
    "        Мы не оказываем поддержки по модифицированной версии игры.\n"
    "===========================================================================\n",
...
void PrintGameVersion(void)
{
    ...
    for (i=0; gameversions[i].description != NULL; ++i)
    {
        ...
        {
            printf("Режим эмуляции исполняемого файла "
                   "\"%s\".\n", gameversions[i].description);
            break;
        }
    }
...
static void D_InitIntroSequence(void)
{
    ...
    else
    {
        ...
        DEH_snprintf(string, sizeof(string), "и");
        ...
        DEH_snprintf(string, sizeof(string), "представляет");
        ...
        DEH_snprintf(string, sizeof(string), "Загрузка...");
...
void D_DoomMain (void)
{
    ...
    if(devparm)
    {
        ...
        DEH_snprintf(msgbuf, sizeof(msgbuf), "Серийный номер wad-файла: %d:", serialnum);
        printf("%s\n", msgbuf);
    }
...
//    W_PrintDirectory();
    ...
    if (modifiedgame)
    {
        ...
        if ( gamemode == shareware)
            I_Error(DEH_String("\nВ демо-версии загрузка файлов "
                               "недоступна. Купите полную версию!"));
    ...
    if(devparm) // [STRIFE]
       DEH_printf("R_Init: Загрузка графики - ");
    R_Init ();
    D_IntroTick(); // [STRIFE]
    ...
    if(devparm) // [STRIFE]
        DEH_printf("M_Init: Инициализация меню.\n");
    M_Init ();
    D_IntroTick(); // [STRIFE]
    ...
    if(devparm)
        DEH_printf("HU_Init: Инициализация HUD.\n");
    HU_Init ();
    D_IntroTick(); // [STRIFE]
