...
{ 
    ...
    M_StringCopy(mission_objective, DEH_String("yfqlb gjvjom"), // [rus] Найди помощь
                 OBJECTIVE_LEN);
}
...
{ 
    ...
    if (!players[playernum].mo)
    {
        ...
        if(leveltime > 0)
            players[playernum].message = DEH_String("ytn ntkf!"); // [rus] Нет тела!
...
{ 
    ...
    {
        M_snprintf(savename, sizeof(savename), "buhf \"%s\" cj[hfytyf>", character_name); // [rus] Игра "%s" сохранена.
