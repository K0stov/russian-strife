...
static void OpenPromptWindow(txt_mouse_input_t *mouse_input)
{
    ...
    window = TXT_MessageBox(NULL, "ìaµ∞Φ≥σ ΩδαΓΦ°≤ ∞√°Φ...");
...
static void GetMouseButtonDescription(int button, char *buf, size_t buf_len)
{
    ...
    {
        case 0:
            M_StringCopy(buf, "ïàéÇí", buf_len);
            break;
        case 1:
            M_StringCopy(buf, "ÅÉÇéÇí", buf_len);
            break;
        case 2:
            M_StringCopy(buf, "æÉàäìíí", buf_len);
            break;
        default:
            M_snprintf(buf, buf_len, "èìÄÅèÇ #%i", button + 1);
            break;
    }
}
