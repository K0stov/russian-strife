...
static const known_joystick_t *GetJoystickType(int index)
{
    ...
    printf("ìσΦτΓσ±≥φ√Θ ≥Φ∩ ΣµεΘ±≥ΦΩα \"%s\" ± %i ∞ΦφΦ-πσΘ∞∩αΣα∞Φ, %i Ωφε∩Ωα∞Φ, %i ⌡α≥Ωα∞Φ\n",
           name, axes, buttons, hats);
    printf("Å≡ε±ⁿßα ∩≡Φ±δα≥ⁿ Φφ⌠ε≡∞α÷Φ■ ε Σαφφε∞ ΣµεΘ±≥ΦΩσ!\n");
...
static void SetJoystickButtonLabel(void)
{
    ...
    {
        name = "ìσ εßφα≡≤µσφε (≤±≥≡εΘ±≥Γε φσ ∩εΣΩδ■≈σφε?)";
...
{
    TXT_MessageBox(NULL, "äµεΘ±≥ΦΩ ΦδΦ πσΘ∞∩αΣ φσ εßφα≡≤µσφ.\n\n"
                         "ôßσΣΦ≥σ±ⁿ, ≈≥ε ≤±≥≡εΘ±≥Γε ≤±≥αφεΓδσφε\n"
                         "Γ ε∩σ≡α÷ΦεφφεΘ ±Φ±≥σ∞σ Φ Φ∞σσ≥\n"
                         "Ωε≡≡σΩ≥φε φα±≥≡εσφφ√σ Σ≡αΘΓσ≡√.");
...
static void CalibrateJoystick(TXT_UNCAST_ARG(widget), TXT_UNCAST_ARG(unused))
{
    ...
    calibration_window = TXT_NewWindow("èαδΦß≡εΓΩα ΣµεΘ±≥ΦΩα/πσΘ∞∩αΣα");

    TXT_AddWidgets(calibration_window,
                   TXT_NewStrut(0, 1),
                   TXT_NewLabel("Ä≥÷σφ≥≡Φ≡≤Θ≥σ D-pad ΦδΦ ΣµεΘ±≥ΦΩ\n"
                                "Φ φαµ∞Φ≥σ δ■ß≤■ Ωφε∩Ω≤."),
                   TXT_NewStrut(0, 1),
                   NULL);
...
void ConfigJoystick(void)
{
    ...
    TXT_AddWidget(window, TXT_NewSeparator("èφε∩ΩΦ"));

    AddJoystickControl(window, "Ç≥αΩα/±≥≡σδⁿßα", &joybfire);
    AddJoystickControl(window, "üεΩε∞ ΓδσΓε", &joybstrafeleft);

    AddJoystickControl(window, "ê±∩εδⁿτεΓα≥ⁿ", &joybuse);
    AddJoystickControl(window, "üεΩε∞ Γ∩≡αΓε", &joybstraferight);

    AddJoystickControl(window, "Å≡σΣ√Σ≤∙σσ ε≡≤µΦσ", &joybprevweapon);
    AddJoystickControl(window, "äΓΦµσφΦσ ßεΩε∞", &joybstrafe);

    AddJoystickControl(window, "æδσΣ≤■∙σσ ε≡≤µΦσ", &joybnextweapon);
    ...
    if (joybspeed < 20)
    {
        AddJoystickControl(window, "æΩε≡ε±≥ⁿ", &joybspeed);
    }

    if (gamemission == hexen || gamemission == strife)
    {
        AddJoystickControl(window, "Å≡√µεΩ", &joybjump);
    }

    AddJoystickControl(window, "ÇΩ≥ΦΓΦ≡εΓα≥ⁿ ∞σφ■", &joybmenu);

    AddJoystickControl(window, "Ä≥Ω≡√≥ⁿ Ωα≡≥≤", &joybautomap);
