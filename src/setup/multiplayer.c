...
static txt_window_action_t *StartGameAction(int multiplayer)
{
    ...
    action = TXT_NewWindowAction(KEY_F10, "çα∩≤±Ω");
...
static void OpenWadsWindow(TXT_UNCAST_ARG(widget), TXT_UNCAST_ARG(user_data))
{
    ...
    window = TXT_NewWindow("äεßαΓΦ≥ⁿ WAD-⌠αΘδ√");

    for (i=0; i<NUM_WADS; ++i)
    {
        TXT_AddWidget(window,
                      TXT_NewFileSelector(&wads[i], 60, "é√ßε≡ WAD-⌠αΘδα",
                                          wad_extensions));
    }
}
                                  ...
                                  TXT_UNCAST_ARG(user_data))
{
    ...
    window = TXT_NewWindow("äε∩εδφΦ≥σδⁿφ√σ ∩α≡α∞σ≥≡√ Ωε∞αφΣφεΘ ±≥≡εΩΦ");
...
static txt_window_action_t *WadWindowAction(void)
{
    ...
    action = TXT_NewWindowAction('w', "WAD-⌠αΘδ√");
...
static void StartGameMenu(char *window_title, int multiplayer)
{
    ...
    TXT_AddWidgets(window,
                   TXT_NewSeparator("ìα±≥≡εΘΩΦ ∞εφ±≥≡εΓ"),
                   TXT_NewInvertedCheckBox("îεφ±≥≡√ Γ Φπ≡σ", &nomonsters),
                   TXT_TABLE_OVERFLOW_RIGHT,
                   TXT_NewCheckBox("ü√±≥≡√σ ∞εφ±≥≡√", &fast),
                   TXT_TABLE_OVERFLOW_RIGHT,
                   TXT_NewCheckBox("îεφ±≥≡√ Γε±Ω≡σ°α■≥± ", &respawn),
                   TXT_TABLE_OVERFLOW_RIGHT,
                   NULL);
    ...
    if (multiplayer)
    {
        TXT_AddWidgets(window,
                       ...
                       TXT_NewInvertedCheckBox("ÉσπΦ±≥≡α÷Φ  φα ÷σφ≥≡αδⁿφε∞ ±σ≡Γσ≡σ",
                                               &privateserver),
                       TXT_TABLE_OVERFLOW_RIGHT,
                       NULL);
    }
    ...
    TXT_AddWidgets(window,
                   TXT_NewButton2("äε∩εδφΦ≥σδⁿφ√σ ∩α≡α∞σ≥≡√...",
...
{
    StartGameMenu("ìα≈α≥ⁿ ±σ≥σΓ≤■ Φπ≡≤", 1);
}

void WarpMenu(void)
{
    StartGameMenu("é√ßε≡ ≤≡εΓφ ", 0);
}
...
static void DoJoinGame(void *unused1, void *unused2)
{
    ...
    {
        TXT_MessageBox(NULL, "éΓσΣΦ≥σ αΣ≡σ± ±σ≡Γσ≡α.");
        return;
    }
...
static txt_window_action_t *JoinGameAction(void)
{
    ...
    action = TXT_NewWindowAction(KEY_F10, "æεσΣΦφΦ≥ⁿ± ");
                               ...
                               TXT_UNCAST_ARG(querydata))
{
    ...
    {
        TXT_MessageBox("ìσΓετ∞εµφε ±εσΣΦφΦ≥ⁿ± .",
                       "êπ≡α φα Σαφφε∞ ±σ≡Γσ≡σ ≤µσ ±≥α≡≥εΓαδα.");
        return;
    }
    ...
    if (querydata->num_players > 0)
    {
        ...
        {
            TXT_MessageBox(NULL,
                           "Å≡σΣ∩εδεµΦ≥σδⁿφε, Φπ≡α φα ±σ≡Γσ≡σ:\n"
                           "\n"
                           "   %s\n"
                           "\n"
                           "φε φσεß⌡εΣΦ∞√Θ IWAD-⌠αΘδ %s φσ φαΘΣσφ!\n"
                           "üστ Σαφφεπε ⌠αΘδα ∩≡Φ±εσΣΦφΦ≥ⁿ± Ω Φπ≡σ φσΓετ∞εµφε.",
...
static void QueryPeriodicCallback(TXT_UNCAST_ARG(results_table))
{
    ...
    if (!NET_Query_Poll(QueryResponseCallback, results_table))
    {
        ...
        if (query_servers_found == 0)
        {
            TXT_AddWidgets(results_table,
                TXT_TABLE_EMPTY,
                TXT_NewLabel("æσ≡Γσ≡√ φσ εßφα≡≤µσφ√."),
                NULL
            );
        }
    }
                               ...
                               TXT_UNCAST_ARG(user_data))
{
    ...
    ServerQueryWindow("ÅεΦ±Ω ±σ≡Γσ≡α Γ êφ≥σ≡φσ≥");
}
...
{
    NET_StartLANQuery();
    ServerQueryWindow("ÅεΦ±Ω ±σ≡Γσ≡α Γ δεΩαδⁿφεΘ ±σ≥Φ");
}
...
void JoinMultiGame(void)
{
    ...
    window = TXT_NewWindow("Å≡Φ±εσΣΦφΦ≥ⁿ±  Ω ±σ≥σΓεΘ Φπ≡σ");
    TXT_SetTableColumns(window, 2);
    ...
    TXT_AddWidgets(window,
                   TXT_NewSeparator("æσ≡Γσ≡"),
                   TXT_NewLabel("æεσΣΦφΦ≥ⁿ±  ± αΣ≡σ±ε∞: "),
                   address_box = TXT_NewInputBox(&connect_address, 30),

                   TXT_NewButton2("ÅεΦ±Ω ±σ≡Γσ≡α Γ êφ≥σ≡φσ≥...",
                                  FindInternetServer, NULL),
                   TXT_TABLE_OVERFLOW_RIGHT,
                   TXT_NewButton2("ÅεΦ±Ω ±σ≡Γσ≡α Γ δεΩαδⁿφεΘ ±σ≥Φ...",
                                  FindLANServer, NULL),
                   TXT_TABLE_OVERFLOW_RIGHT,
                   TXT_NewStrut(0, 1),
                   TXT_TABLE_OVERFLOW_RIGHT,
...
void MultiplayerConfig(void)
{
    ...
    window = TXT_NewWindow("ìα±≥≡εΘΩΦ ±σ≥σΓεΘ Φπ≡√");
    TXT_SetWindowHelpURL(window, MULTI_CONFIG_HELP_URL);

    TXT_AddWidgets(window,
                   TXT_NewStrut(0, 1),
                   TXT_NewHorizBox(TXT_NewLabel("ê∞  Φπ≡εΩα:  "),
                                   TXT_NewInputBox(&net_player_name, 25),
                                   NULL),
                   TXT_NewStrut(0, 1),
                   TXT_NewSeparator("îαΩ≡ε±√ Σδ  ≈α≥α"),
                   NULL);
